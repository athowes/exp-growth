...
What about more generally: what is the distribution over time to first detection?
...
compute_alpha(1, 1, c = c, n0 = 1, r = r)
```

So, for example, the probability that a case is observed on day one, using this formula, is `r compute_alpha(1, 1, c = c, n0 = 1, r = r)`.
We can plot the probability that at least one case is observed by day as follows

```{r}
alpha <- tibble(t = seq(1, 2000)) %>%
  mutate(
    cdf = map_dbl(t, ~ compute_alpha(.x, 1, c = c, n0 = 1, r = r)),
    pdf = c(0, diff(cdf))
  )
...
This looks like some interesting behaviour.
After the first few detections, the model thinks that $\beta > 0$, but after a while without any further detections it returns to thinking that $\beta = 0$ is plausible again for a short time, before detections really start to take off.
