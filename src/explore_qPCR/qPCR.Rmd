...
$$
\frac{\text{d}n_c}{\text{d}c} = - (n_0 - n_{\inf}) \frac{1}{\left(1 + (c / m)^\beta \right)^2} \beta \frac{c^{\beta - 1}}{m^\beta}.
$$
Evaluating this at the midpoint, $m$, gives
$$
\frac{\text{d}n_c}{\text{d}c} \rvert_{c = m} = - (n_0 - n_{\inf}) \cdot \frac{\beta}{4m} = \frac{(n_{\inf} - n_0)}{4m} \cdot \beta,
$$
such that the derivative is proportional to the slope parameter $\beta$.
...
$$
n_c = n_\infty + \frac{n_0 - n_{\inf}}{\left(1 + (c / m)^\beta \right)^s}
$$
