...
We will now simulate and plot data for this model (lognormal geometric auto-regressive for the baseline, and lognormal geometric random walk^[Draws from Gaussian a growth rate with mean greater than zero (exponential regime) can be quite varied, so might be worth thinking about changing this too.] for the exponential) with `r n_species` species in total, each of which with `r n_kmer` $k$-mers as before.
